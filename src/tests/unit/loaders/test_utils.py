...
class TestUtils(unittest.TestCase):
    def setUp(self):
        ...
        """
        with open(
            self.description_path, "w", encoding=DESCRIPTION_CONFIGS["encoding"]
        ) as file:
    ...
    def test_clean_html_text(self):
        with open(
            self.description_path, "r", encoding=DESCRIPTION_CONFIGS["encoding"]
        ) as file:
        ...
        # Test normalization
        assert re.search(
            DESCRIPTION_CONFIGS["section_name"]["pattern"], cleaned1, re.IGNORECASE
        ) or re.search(
            DESCRIPTION_CONFIGS["sas_variable_name"]["pattern"], cleaned1, re.IGNORECASE
        )
        cleaned2 = re.sub(
        ...
        assert (
            spaces_name_after == 2
        )  # Do not forget that it counts here for "Section Name: " and "SAS Variable Name: ", so it's 2 # NOQA E501
